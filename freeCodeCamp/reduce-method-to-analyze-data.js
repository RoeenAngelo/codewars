...
function getRating(watchList) {
  const filtered = watchList.filter(item => item.Director === "Christopher Nolan").map(item => +item.imdbRating)

  const averageRating = filtered.reduce((a,c) => a + c, 0) / filtered.length;
